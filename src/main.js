...
function tick(step){
    ...
    if(nowCpu - lastCpu > 1){
        logger.error('CPU Usage '+step+': '+ printFloat(nowCpu - lastCpu)+' total:'+printFloat(nowCpu),'CPUTrack');
    }else{
        logger.trace('CPU Usage '+step+': '+ printFloat(nowCpu - lastCpu)+' total:'+printFloat(nowCpu),'CPUTrack');
    }
    lastCpu = nowCpu;
}
